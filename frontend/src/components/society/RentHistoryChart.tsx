...
export function RentHistoryChart({ societySlug }: { societySlug: string }) {
  ...
  return <section className="mt-8 rounded-[28px] border border-blue-100 bg-white p-5 shadow-sm md:p-7">
    ...
    <svg viewBox="0 0 100 100" className="mt-5 h-52 w-full overflow-visible" role="img" aria-label="Median rent trend"><path d={`M ${points.map((p) => `${p.x} ${p.y}`).join(' L ')}`} fill="none" stroke="#2563eb" strokeWidth="2.5" vectorEffect="non-scaling-stroke" />{points.map((p) => <g key={p.id}><circle cx={p.x} cy={p.y} r="2.2" fill="#2563eb" /><text x={p.x} y={Math.max(8, p.y - 6)} textAnchor="middle" fontSize="4" fill="#0f172a">{money(p.median_rent)}</text><text x={p.x} y="98" textAnchor="middle" fontSize="3.3" fill="#64748b">{new Date(p.recorded_on).toLocaleDateString('en-IN', { month: 'short', year: '2-digit' })}{p.bhk ? ` · ${p.bhk}BHK` : ''}</text></g>)}</svg>
