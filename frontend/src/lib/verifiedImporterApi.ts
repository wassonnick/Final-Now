...
export const setVerifiedImportCoverImage = (id: number, replace = false) => adminFetch(`/admin/verified-importer/images/${id}/set-cover`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ replace }) }).then(json);
...
export const generateVerifiedSocietySeo = (societyId: number, replace = false) => adminFetch(`/admin/verified-importer/societies/${societyId}/generate-seo`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ replace }) }).then(json);
export const generateVerifiedSocietyScores = (societyId: number, replace = false) => adminFetch(`/admin/verified-importer/societies/${societyId}/generate-scores`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ replace }) }).then(json);
export const applyVerifiedSocietyMarketData = (societyId: number) => adminFetch(`/admin/verified-importer/societies/${societyId}/apply-market-data`, { method: "POST" }).then(json);
