...
import { CalendarDays, Download, Eye, Mail, MessageCircle, Phone, RefreshCw, Search, Trash2, UserCheck } from 'lucide-react';
...
export function AdminLeadsPage() {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((lead) => {
      const matchesSearch = !search || [lead.name, lead.phone, lead.email, lead.society, lead.property, lead.budget, lead.assignedTo, lead.source, lead.requirement]
        .join(' ')
        .toLowerCase()
        .includes(search);
  ...
  return (
    <AdminLayout title="Leads CRM" subtitle="Track enquiries, follow-ups, site visits and conversion pipeline">
      <div className="space-y-6">
        {error ? (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">{error}</div>
        ) : null}
        ...
        <section className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Live backend leads from SocietyFlats public enquiries and callback forms.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Button onClick={loadLeads} variant="outline" className="rounded-full border-slate-200" disabled={loading}>
          ...
          <div className="mt-5 overflow-hidden rounded-[28px] border border-slate-200">
            ...
            {loading ? (
              <div className="bg-white px-5 py-12 text-center text-sm text-slate-500">Loading live backend leads...</div>
            ) : null}
              ...
              <div key={lead.id} className="grid gap-4 border-t border-slate-200 bg-white px-5 py-5 lg:grid-cols-[1.3fr_1.1fr_0.9fr_0.8fr_0.8fr_190px] lg:items-center">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-sm text-slate-500">
                    <a href={`tel:${lead.phone.replace(/\s/g, '')}`} className="inline-flex items-center gap-1 hover:text-blue-700"><Phone className="h-3.5 w-3.5" /> {lead.phone || 'No phone'}</a>
                    {lead.email ? <a href={`mailto:${lead.email}`} className="inline-flex items-center gap-1 hover:text-blue-700"><Mail className="h-3.5 w-3.5" /> Email</a> : null}
                  </div>
                  <p className="mt-2 text-xs font-medium text-slate-400">Source: {lead.source}</p>
                </div>
                ...
                <div className="text-sm text-slate-600">
                  <div className="flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5 text-slate-400" /> {lead.followUpAt || 'Not set'}</div>
