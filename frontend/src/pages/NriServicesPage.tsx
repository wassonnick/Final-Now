...
const nriLanes = [
  {
    ...
    body: "Shortlist societies, verify locality fit, compare towers and coordinate visits for family or representatives across Delhi NCR.",
    icon: Home,
  },
  {
    ...
    <main className="ncr-skin min-h-screen bg-[#F7F2EA] text-[#111827]">
      <section className="border-b border-[#E6DDCF] bg-[radial-gradient(circle_at_82%_12%,rgba(194,114,78,.16),transparent_30%),radial-gradient(circle_at_12%_20%,rgba(35,59,110,.12),transparent_28%),linear-gradient(180deg,#FFFCF7,#F7F2EA)]">
        <div className="mx-auto grid max-w-[1360px] gap-9 px-5 py-14 md:px-10 md:py-20 lg:grid-cols-[1fr_430px] lg:items-center">
          <div>
            ...
            <h1 className="!font-sans mt-5 max-w-5xl text-[40px] font-semibold leading-[1.03] tracking-[-0.025em] md:text-[64px]">
          ...
          <form id="nri-consultation" onSubmit={submit} className="rounded-[28px] border border-[#E4E4E9] bg-white p-6 shadow-[0_30px_80px_-52px_rgba(17,24,39,.5)]">
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-[14px] bg-[#ECF6F2] text-[#0F7B63]"><MessageSquareText className="h-5 w-5" /></span>
              <div>
                <h2 className="!font-sans text-[19px] font-semibold text-[#1D1D1F]">Start an NRI case</h2>
                <p className="text-[12.5px] text-[#6E6E73]">Free · no passport, bank or tax details needed</p>
              </div>
            </div>
            <div className="mt-5 grid gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <select value={form.service_type} onChange={(e) => setForm({ ...form, service_type: e.target.value })} className="h-10 rounded-md border border-[#E4E4E9] bg-white px-3 text-sm">
                  {serviceOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                </select>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <Input type="email" placeholder="Email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
                <Input placeholder="WhatsApp (with code)" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
              </div>
              <textarea
                className="min-h-24 rounded-md border border-[#E4E4E9] p-3 text-sm"
                placeholder="What do you need? e.g. rent out my apartment, sell a family flat, buy near an office hub."
                value={form.notes}
                onChange={(e) => setForm({ ...form, notes: e.target.value })}
              />
              <label className="flex items-start gap-2 text-[12.5px] leading-5 text-[#6E6E73]">
                <input required type="checkbox" checked={form.consent} onChange={(e) => setForm({ ...form, consent: e.target.checked })} className="mt-0.5" />
                I consent to SocietyFlats contacting me about this request.
              </label>
              <Button disabled={saving} className="h-11 rounded-full bg-[#0F7B63] font-semibold text-white hover:bg-[#0C6853]">{saving ? "Submitting…" : "Submit NRI case"}</Button>
            </div>
            {error ? <p className="mt-3 rounded-[12px] bg-rose-50 p-3 text-sm font-semibold text-rose-700">{error}</p> : null}
            {notice ? <p className="mt-3 rounded-[12px] bg-[#ECF6F2] p-3 text-sm font-semibold text-[#0F7B63]">{notice}</p> : null}
          </form>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1360px] px-5 py-12 md:px-10">
        ...
        <section className="mt-10 grid gap-7 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <aside className="rounded-[30px] border border-[#E4E4E9] bg-[#F5F5F7] p-6 md:p-8">
            <ShieldCheck className="h-7 w-7 text-[#0F7B63]" />
            <h2 className="!font-sans mt-4 text-2xl font-semibold md:text-3xl">Clear scope, fewer surprises.</h2>
            <ul className="mt-5 space-y-3 text-sm leading-6 text-[#6E6E73]">
            ...
            <div className="mt-6 rounded-[20px] bg-white p-5">
              <p className="text-sm font-bold text-[#1D1D1F]">Already have a society in mind?</p>
              <p className="mt-1 text-sm leading-6 text-[#6E6E73]">Open the profile first, then send us the exact requirement so the case starts with better context.</p>
              <Link to="/search?tab=societies" className="mt-4 inline-flex items-center text-sm font-semibold text-[#0F7B63]">
