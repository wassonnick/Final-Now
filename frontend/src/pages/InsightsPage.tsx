...
export function InsightsPage() {
  ...
  return (
    <div className="ncr-skin min-h-screen bg-[#F8F3EA]">
      <section className="border-b border-[#E4E4E9] bg-white px-4 py-10 md:py-14">
        <div className="container mx-auto max-w-[1100px]">
          <span className="inline-flex items-center gap-2 rounded-full border border-[#E4E4E9] bg-white px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.16em] text-[#0F7B63] shadow-sm">
            <TrendingUp className="h-3.5 w-3.5" />
            Source-labelled intelligence
          </span>

          <h1 className="!font-sans mt-4 max-w-3xl text-[34px] font-semibold leading-[1.05] tracking-[-0.02em] text-[#1D1D1F] md:text-[52px]">
            Gurgaon rent &amp; resale, with the confidence behind every number.
          </h1>

          <p className="mt-4 max-w-2xl text-[16px] leading-8 text-[#6E6E73]">
            Honest price bands, demand shifts and investment context across Gurgaon's key micro-markets — so you can time your move with a clear head.
          </p>

          {/* Lens — the primary control */}
          <div className="mt-7 inline-flex rounded-full border border-[#E4E4E9] bg-[#F5F5F7] p-1">
            {(["rent", "buy", "sell"] as InsightMode[]).map((item) => (
              <button
                ...
                className={
                  mode === item
                    ? "rounded-full bg-[#0F7B63] px-5 py-2 text-sm font-semibold capitalize text-white"
                    : "rounded-full px-5 py-2 text-sm font-semibold capitalize text-[#6E6E73] hover:text-[#1D1D1F]"
                }
              >
          ...
          {/* Source honesty + primary CTA */}
          <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="flex items-start gap-2 text-[13.5px] leading-6 text-[#6E6E73] sm:max-w-xl">
              <Info className="mt-0.5 h-4 w-4 shrink-0 text-[#0F7B63]" />
              Every range is pulled live from published society profiles — the actual source and confidence show on each row. Nothing here is a static guess.
            </p>
            <Link to={modeHref(mode)} className="inline-flex shrink-0 items-center gap-2 rounded-full bg-[#0F7B63] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0C6853]">
              {activeCopy.cta} <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
      </section>

      <section className="container mx-auto max-w-[1100px] px-4 py-8">
        {/* Slim trust strip — facts, not filler cards */}
        <div className="flex flex-wrap gap-x-8 gap-y-3 rounded-2xl border border-[#E4E4E9] bg-white px-5 py-4">
          {heroMetrics.map(([label, value]) => (
            <div key={label} className="min-w-[120px]">
              ...
              <p className="mt-0.5 text-[17px] font-semibold text-[#1D1D1F]">{value}</p>
            </div>
          ))}
        </div>

        <div className="mt-6 rounded-[1.75rem] border border-[#E4E4E9] bg-white p-4 shadow-sm">
