...
{
    public function configured(): bool{return filled(config('services.search_console.access_token'))&&filled(config('services.search_console.site_url'));}
    ...
    public function fetch(int $days=28): int
    {
        ...
        $response=Http::withToken((string)config('services.search_console.access_token'))->timeout(30)->post("https://searchconsole.googleapis.com/webmasters/v3/sites/{$site}/searchAnalytics/query",['startDate'=>now()->subDays($days)->toDateString(),'endDate'=>now()->subDay()->toDateString(),'dimensions'=>['date','page','query'],'rowLimit'=>25000]);
