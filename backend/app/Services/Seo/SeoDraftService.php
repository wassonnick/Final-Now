...
class SeoDraftService
{
    ...
    public function publish(SeoDraft $draft,string $actor): SeoDraft
    {
        ...
        if($page->page_type==='society'&&$page->entity_type===Society::class&&$page->entity_id){$content=SocietySeoContent::where('society_id',$page->entity_id)->firstOrFail();if($content->status!=='approved')throw new \InvalidArgumentException('The linked society SEO content is not approved.');$content->update(['status'=>'published','published_at'=>now()]);$contentId=$content->id;}
