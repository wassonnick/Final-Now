...
class SocietyController extends Controller {
  ...
  public function lookup(Request $request): JsonResponse {
    ...
    $query = Society::query()
      ...
      ->when($request->filled('locality_id'), fn ($q) => $q->where('locality_id', $request->string('locality_id')))
      ->when($request->filled('city'), fn ($q) => $q->where('city', 'ilike', (string) $request->query('city')))
      ->when($request->filled('city_slug'), fn ($q) => $q->whereHas('cityRecord', fn ($city) => $city->where('slug', (string) $request->query('city_slug'))))
  ...
  public function show(string $idOrSlug): JsonResponse
{
  ...
  private function applyNcrLocationFilters($query, Request $request): void
  {
    ...
    if ($request->filled('zone_id')) $query->where('zone_id', $request->integer('zone_id'));
    if ($request->filled('locality_id')) $query->where('locality_id', $request->string('locality_id'));
    if ($request->filled('city')) $query->where('city', 'ilike', (string) $request->query('city'));
    if ($request->filled('city_slug')) $query->whereHas('cityRecord', fn ($city) => $city->where('slug', (string) $request->query('city_slug')));
  }
